let apiweatherURL = "https://api.openweathermap.org/data/2.5/weather?lat=42.0380399&lon=-111.4048681&appid=ef2ec14cd00cc3ff1646332d4b42644d&units=imperial"
...
let apiforecastURL = "https://api.openweathermap.org/data/2.5/forecast?id=5585010&appid=ef2ec14cd00cc3ff1646332d4b42644d&units=imperial"
...
fetch(requestURL)
  ...
  .then(function (jsonObject) {
    ...
    const townProject = towns.filter(town => (town.name == "Fish Haven"));
    townProject.forEach(town => {
      ...
      let townEventFour = document.createElement("p");
      ...
      townEventFour.textContent = town.events[3];

      card.appendChild(townName);
      card.appendChild(townEventOne);
      card.appendChild(townEventTwo);
      card.appendChild(townEventThree);
      card.appendChild(townEventFour);

      document.querySelector(".fishhaven-events").appendChild(card);
    });
